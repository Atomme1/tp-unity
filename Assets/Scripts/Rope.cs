...
public class Rope : MonoBehaviour
{
    ...
    void CreateRope()
    {
        ...
        //Les extr�mit�s ne sont pas libres
    ...
    void Simulate()
    {
        ...
        //Initialisation de la position des extr�mit�s.
    ...
    {
        //Partie 3.
        points[i].position= points[i].position + (points[i].position - points[i].oldPosition) * 0.3f;
    }
    ...
    void ApplyDistanceConstraint(int pointA, int pointB, float desiredSegmentLength)
    {
        ...
        if(points[pointA].isFree && points[pointB].isFree){
            ...
            line.SetPosition(pointA, points[pointA].position);
            line.SetPosition(pointB, points[pointB].position);

        }
        else if(points[pointB].isFree){

            line.SetPosition(pointB, points[pointB].position * desiredSegmentLength);

        }
        else if(points[pointA].isFree){
            line.SetPosition(pointA, points[pointA].position * desiredSegmentLength);
